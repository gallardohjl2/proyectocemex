...
type
  TFormConsultasDetalle = class(TForm)
    ...
    procedure CargarDatosBatchTable(ALote: String);
...
begin
  ...
  if DataModule1.ADOQueryTotalLab.Locate('Lote', Lote, []) then
  begin
    ActualizarCamposDesdeTotalLab;
  end
  else
  begin
    ShowMessage('No se encontró el Lote ingresado.');
...
procedure TFormConsultasDetalle.CargarDatosBatchTable(ALote: string);
begin
  try
    ...
    DataModule1.ADOQueryBatchTable.SQL.Add('SELECT * FROM aditivos.batchtable where Lote like '+QuotedStr(ALote+'%'));
...
var
  Lote: string;
begin
  // leemos el campo Lote de TotalLab
  Lote := DataModule1.ADOQueryTotalLab.FieldByName('Lote').AsString;

  // el edit visual se sigue llamando EditOrdenFab, pero muestra el lote
  EditOrdenFab.Text       := Lote;
  ...
  // ahora mandamos el Lote a BatchTable
  CargarDatosBatchTable(Lote);
end;
